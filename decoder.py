...
class Decoder(nn.Module):
    ...
    def linkpred(self, block_outputs, pos_graph, neg_graph):
        ...
        #     neg_emb_l = []
        #     for i in range(self.args.neg_list_num):
        #         with neg_graph[i].local_scope():
        #             neg_graph[i].ndata['feat'] = block_outputs
        #             neg_graph[i].apply_edges(linkpred_concat)
        #             neg_emb_l.append(neg_graph[i].edata['emb'])
        #     neg_emb = torch.cat([neg_emb_l[n] for n in range(self.args.neg_list_num)])
        # else:
